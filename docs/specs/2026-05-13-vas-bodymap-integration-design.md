...
| `src/features/evaluations/domain/schema.ts` | 수정 | `toggleVas`, `vas` 필드 + 검증 제거 |
| `src/features/evaluations/domain/types.ts` | 수정 | `PainArea.id`에 `'general'` 가능 명시 (또는 `string` 그대로 유지) |
| `src/data/body-parts.ts` | 수정 | 'general' 부위 ID + 라벨 추가 |
