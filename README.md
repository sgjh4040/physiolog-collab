...
## 친구(물리치료사) 인계용 설치 (zip 배포)

Node.js 20+ 설치 후:

- **Mac**: 압축 풀고 `setup.command` 더블클릭 (또는 터미널 `bash setup.sh`)
- **Windows**: 압축 풀고 `setup.bat` 더블클릭

설치 후 자동으로 http://localhost:3000 으로 브라우저 열림. 폰에서 보려면 같은 와이파이에 연결 후 터미널에 뜬 `Network: http://192.168.xxx.xxx:3000` 주소 입력 → "홈 화면에 추가"로 PWA 설치.
